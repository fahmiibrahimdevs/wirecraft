      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-4 md:px-6 md:py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-sky-500/10 dark:bg-sky-500/20 border border-sky-500/30 flex items-center justify-center text-sky-600 dark:text-sky-400 shadow-xs">
              <TableIcon className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base md:text-lg font-bold text-slate-900 dark:text-slate-100">
                  Tabel Wiring Pin Hardware
                </h2>
                <span className="text-xs font-semibold bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20 px-2.5 py-0.5 rounded-full">
                  {totalPins} Jalur Terpetakan
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Pemetaan pinout mikrokontroler, antarmuka sinyal, level converter, dan GPIO fisik secara otomatis
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 active:scale-98 text-white shadow-xs transition-all cursor-pointer"
              title="Unduh tabel dalam format CSV (Cocok untuk Google Sheets / Excel)"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition-colors cursor-pointer border border-slate-200 dark:border-slate-700"
            ...
            >
              ...
              <span className="hidden sm:inline">{copiedType === 'markdown' ? 'Tersalin!' : 'Salin Markdown'}</span>
            </button>
        ...
        <div className="px-4 md:px-6 py-2.5 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-slate-900">
          {/* Tab Selector per MCU (like Google Sheets bottom tabs) */}
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 max-w-full">
            ...
            >
              ...
              <span>Semua MCU ({totalPins})</span>
            </button>

            {groupsWithCustomNotes.map((g, idx) => (
              <button
                key={g.mcu.id}
                onClick={() => setActiveTab(g.mcu.id)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap ${
                  activeTab === g.mcu.id
                    ? 'bg-sky-500 text-white shadow-xs'
                    : 'bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
              ...
              >
                ...
                <span>
                  MCU {idx + 1}: {g.mcu.label || g.mcu.name || g.mcuDef.name} ({g.rows.length})
                </span>
              </button>
            ))}
          </div>
          ...
          <div className="relative min-w-[200px] sm:w-64">
            ...
            <input
              type="text"
              placeholder="Cari pin, sensor, GPIO, interface..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-hidden focus:border-sky-500 transition-colors font-normal"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
          {filteredGroups.length === 0 ? (
            ...
            <div className="py-16 text-center space-y-3">
              <div className="w-14 h-14 mx-auto rounded-2xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400">
                <TableIcon className="w-7 h-7" />
              </div>
              <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                {groups.length === 0
                  ? 'Belum Ada Mikrokontroler / Kabel Terhubung'
                  : 'Tidak Ada Baris yang Cocok dengan Pencarian'}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                {groups.length === 0
                  ? 'Tambahkan mikrokontroler (Arduino, ESP32, NodeMCU, Wemos) dan hubungkan kabel ke sensor/modul untuk melihat tabel pemetaan pin hardware otomatis.'
                  : 'Coba ubah kata kunci pencarian pada kotak pencarian di atas.'}
              </p>
            </div>
          ) : (
            /* Grouped Tables by MCU */
            filteredGroups.map((group) => (
              <div
                key={group.mcu.id}
                className="border border-slate-200 dark:border-slate-800/80 rounded-xl overflow-hidden shadow-xs bg-white dark:bg-slate-900/60 transition-colors"
              >
                {/* Header Group Banner */}
                <div className="px-4 py-3 bg-slate-100/80 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700/80 flex items-center justify-between flex-wrap gap-2">
                  ...
                  <span className="text-xs font-semibold bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-lg shadow-2xs">
                    {group.rows.length} Pin Terhubung
                  </span>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs">
                      {group.rows.map((row) => (
                        <tr
                          key={row.id}
                          onMouseEnter={() => {
                            if (onHighlightWire && row.wireId) onHighlightWire(row.wireId);
                            if (onHighlightComponent && row.targetCompId) onHighlightComponent(row.targetCompId);
                          }}
                          onMouseLeave={() => {
                            if (onHighlightWire) onHighlightWire(null);
                            if (onHighlightComponent) onHighlightComponent(null);
                          }}
                          className="hover:bg-sky-50/60 dark:hover:bg-sky-950/30 transition-colors group cursor-default"
                        >
                          {/* No */}
                          <td className="py-2.5 px-3 text-center text-xs font-medium text-slate-400 dark:text-slate-500">
                            {row.no}
                          </td>
                            ...
                            <div className="flex items-center gap-2">
                              {row.wireColor && (
                                <span
                                  ...
                                  title={`Warna kabel: ${row.wireColor}`}
                                />
                              )}
                          ...
                          <td className="py-2.5 px-3">
                            <span
                              className={`text-[11px] font-medium px-2 py-0.5 rounded-md border inline-flex items-center whitespace-nowrap ${getInterfaceBadgeStyle(
                                row.interfaceType
                              )}`}
                            >
                              {row.interfaceType}
                          ...
                          {/* Pin MCU */}
                          <td className="py-2.5 px-3">
                            <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 px-2 py-0.5 rounded-md inline-block">
                              {row.mcuPin}
                            </span>
                          </td>
                          ...
                          <td className="py-2.5 px-3">
                            {row.gpio && row.gpio !== '-' ? (
                              <span className="text-xs font-semibold text-sky-600 dark:text-sky-400 bg-sky-500/10 dark:bg-sky-500/15 border border-sky-500/20 px-2 py-0.5 rounded-md inline-block">
                                {row.gpio}
                              </span>
                            ) : (
                          ...
                          <td className="py-2.5 px-3 text-center">
                            {row.pinLv !== '-' ? (
                              <span className="bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/20 px-2 py-0.5 rounded-md text-xs font-semibold inline-block">
                                {row.pinLv}
                              </span>
                            ) : (
                              <span className="text-slate-300 dark:text-slate-600 text-xs font-medium">—</span>
                          ...
                          <td className="py-2.5 px-3 text-center">
                            {row.converterModule !== '-' ? (
                              <span className="bg-purple-500/15 text-purple-600 dark:text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-md text-xs font-medium inline-block">
                                {row.converterModule}
                              </span>
                            ) : (
                              <span className="text-slate-300 dark:text-slate-600 text-xs font-medium">—</span>
                          ...
                          <td className="py-2.5 px-3 text-center">
                            {row.pinHv !== '-' ? (
                              <span className="bg-rose-500/15 text-rose-600 dark:text-rose-400 border border-rose-500/20 px-2 py-0.5 rounded-md text-xs font-semibold inline-block">
                                {row.pinHv}
                              </span>
                            ) : (
                              <span className="text-slate-300 dark:text-slate-600 text-xs font-medium">—</span>
                          ...
                          <td className="py-2.5 px-3">
                            <input
                              ...
                              className="w-full bg-transparent hover:bg-slate-100/80 dark:hover:bg-slate-800/60 focus:bg-white dark:focus:bg-slate-800 px-2 py-1 rounded-md text-xs text-slate-700 dark:text-slate-300 placeholder-slate-400/60 border border-transparent focus:border-sky-500 outline-hidden transition-colors"
                            />
                          </td>
                        </tr>
                      ))}
        ...
        <div className="p-3 md:px-6 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex items-center gap-2">
            ...
            <span>Arahkan mouse pada baris untuk menyalakan kabel & komponen terkait di kanvas.</span>
          </div>
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-lg font-semibold bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 transition-colors cursor-pointer"
          >
            Tutup
          </button>
        </div>
