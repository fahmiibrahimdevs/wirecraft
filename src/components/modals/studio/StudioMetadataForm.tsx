    ...
    <div className="w-80 bg-slate-50 dark:bg-slate-950/60 border-r border-slate-200 dark:border-slate-800 p-4 flex flex-col gap-4 overflow-y-auto">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="p-3 bg-white dark:bg-slate-900/80 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col gap-2.5 shadow-xs">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <button
                  onClick={onMagicRemoveBackground}
      ...
      <div className="flex flex-col gap-2.5 pt-2 border-t border-slate-200 dark:border-slate-800">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-950 p-0.5 rounded-lg border border-slate-200 dark:border-slate-800">
              ...
              <button
                ...
                title="Gunakan satuan Pixels (px) Canvas"
              >
                px
              </button>
            </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
              <span>Lebar ({unit})</span>
              <span className="font-mono text-[9px] text-slate-400">
                {unit === 'mm' ? `${width}px` : `${pxToMm(width)}mm`}
              </span>
            </span>
            <input
              type="number"
              min="5"
              step={unit === 'mm' ? '0.1' : '1'}
              value={unit === 'mm' ? pxToMm(width) : width}
              onChange={(e) => {
                const val = Number(e.target.value);
                const pxVal = unit === 'mm' ? mmToPx(val) : val;
                const aspect = width > 0 && height > 0 ? height / width : 1;
                setWidth(pxVal);
                if (lockAspectRatio && pxVal > 0) {
                  setHeight(Math.round(pxVal * aspect * 10) / 10);
                }
              }}
              className="px-2.5 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono font-semibold text-slate-800 dark:text-slate-100"
            />
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
              <span>Tinggi ({unit})</span>
              <span className="font-mono text-[9px] text-slate-400">
                {unit === 'mm' ? `${height}px` : `${pxToMm(height)}mm`}
              </span>
            </span>
            <input
              type="number"
              min="5"
              step={unit === 'mm' ? '0.1' : '1'}
              value={unit === 'mm' ? pxToMm(height) : height}
              onChange={(e) => {
                const val = Number(e.target.value);
                const pxVal = unit === 'mm' ? mmToPx(val) : val;
                ...
                setHeight(pxVal);
                if (lockAspectRatio && pxVal > 0) {
                  setWidth(Math.round(pxVal * aspect * 10) / 10);
                }
              }}
              className="px-2.5 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono font-semibold text-slate-800 dark:text-slate-100"
            />
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-600 dark:text-slate-400">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer" title="Gerakkan Pin & Gambar bersamaan saat tombol panah ditekan">
            <input
              type="checkbox"
              checked={linkPinsToImage}
              onChange={(e) => setLinkPinsToImage(e.target.checked)}
              className="rounded text-sky-500 focus:ring-0 w-3.5 h-3.5"
            />
        ...
        <div className="p-2.5 bg-white dark:bg-slate-900/80 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col gap-2 shadow-xs">
          <div className="flex items-center justify-between text-[10px] font-semibold text-slate-700 dark:text-slate-300">
            <span>Fine Alignment (Nudge)</span>
            <span className="font-mono text-slate-400">
              Offset: {imageOffset.x.toFixed(1)}, {imageOffset.y.toFixed(1)} px
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-1 w-28 mx-auto">
            <div />
            <button
              onClick={() => (linkPinsToImage ? nudgeAll(0, -1) : nudgeImage(0, -1))}
              className="p-1.5 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 flex items-center justify-center transition-colors cursor-pointer"
            >
              <ArrowUp className="w-3.5 h-3.5" />
            </button>
            <div />
            <button
              onClick={() => (linkPinsToImage ? nudgeAll(-1, 0) : nudgeImage(-1, 0))}
              className="p-1.5 rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 flex items-center justify-center transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className="p-1 text-[9px] font-bold rounded bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 flex items-center justify-center cursor-pointer"
              title="Reset Offset Gambar ke 0,0"
            >
              0,0
            </button>
            <button
      ...
      <div className="flex flex-col gap-2.5 pt-2 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            type="text"
            value={typeId}
            onChange={(e) => setTypeId(e.target.value.toLowerCase().replace(/[^a-z0-9-_]/g, ''))}
            className="px-2.5 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono font-semibold text-sky-600 dark:text-sky-400"
            placeholder="custom-sensor-mq2"
          />
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="px-2.5 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-semibold text-slate-800 dark:text-slate-100"
            placeholder="Sensor Gas MQ-2"
          />
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            className="px-2.5 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-800 dark:text-slate-100 cursor-pointer"
          >
            {CATEGORIES.map((cat) => (
        ...
        <div className="flex flex-col gap-1">
          ...
          <textarea
            ...
            rows={2}
            className="px-2.5 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-800 dark:text-slate-100 resize-none"
            placeholder="Modul sensor gas analog/digital dengan potesio"
          />
        </div>
      </div>
    </div>
